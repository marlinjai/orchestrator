...
**Best-of-N (`--best-of N`): held-out-certified selection.** `orchestrator start --best-of N ...` runs N independent attempts of the same goal, each in its own worktree / branch (`orchestrator/<task-id>-attempt-<k>`), then SELECTS the winner using the HELD-OUT verifier ONLY (the lowest `time_to_verified_ms` among attempts that reached `completed` with a held-out PASS), never the Worker-visible in-tree `verify`. **best-of-N REQUIRES a held-out verifier** (a registry `held_out_verify` or a `--held-out` override): with none resolvable it REFUSES and runs zero attempts, by design (without an out-of-reach signal, "best of N" would just amplify the most convincing reward-hack). A held-out fail only removes that attempt from selection; it is never fed back as a retry. Composes with `--held-out` and the stakes gate (a tier-3+ repo still needs `--confirm-stakes`; best-of-N does not relax it). `--best-of 1` (the default) is the normal single-attempt run, unchanged. The cohort record (per-attempt `{branch, status, held_out, time_to_verified_ms, selected}` + the selected branch or escalation) is written to `<state_dir>/cohort.json`.

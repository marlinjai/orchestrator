...
5. Trust anchor + held-out verifier: at run start, `repo_registry.py` resolves an operator-owned policy for the project by its REAL git remote (`git remote get-url`, normalized: un-fakeable by the goal file), from `~/.config/orchestrator/repos.toml` (override `ORCHESTRATOR_REPOS_CONFIG`; example at `docs/repos.example.toml`). The policy holds `held_out_verify` (a command running a test set outside the Worker's reach), `stakes_tier` (1-4), and `allowed_mcp_servers`. These are stored on state (`repo_remote`, `held_out_verify`, `stakes_tier`) and surfaced in `orchestrator status`. `allowed_mcp_servers` is enforced as a per-repo ceiling on the Worker's MCP servers (threaded into `build_worker_options`, see point 1). `stakes_tier` is a real DISPATCH GATE: a repo at `stakes_tier >= STAKES_GATE_THRESHOLD` (3) makes `run_orchestrator` refuse to start (`status=stopped`, "stakes gate" exit_reason, no Worker turn) unless the operator authorizes it via `--confirm-stakes` / `ORCHESTRATOR_CONFIRM_STAKES=1` (`cfg.confirm_stakes`, operator-owned, never goal-file-authored). The `autonomous-orchestration` skill forbids Claude from self-authorizing tier-3+. This does not relax `irreversible_ops` (still hard-escalated in the Marlin Proxy). A malformed registry fails the run loud. The goal file can never author these fields (the red-team's core requirement). On a stop-candidate, after the in-tree verify passes and the tamper tripwire clears (or as the sole gate when there is no in-tree verify), the held-out gate (`held_out.py`, reusing `verify.run_verify`) runs `held_out_verify`: a pass corroborates the green and completes; a FAIL is the reward-hack fingerprint (visible green, out-of-reach red) and escalates, recorded as `state.last_held_out` and surfaced in `ground_truth_summary`. A held-out fail is NEVER a Worker retry (that would teach to the hidden tests). The filesystem isolation (held-out tests on a path the Worker's OS user cannot write) is the operator's setup, not the orchestrator's: the orchestrator only guarantees the command is operator-sourced (registry, not goal file) and runs it. For one-off / dogfood runs an operator can supply an ad-hoc held-out via the `--held-out` flag (`cfg.held_out_override`, same operator trust: a goal file still cannot set it). It can ADD a gate to an unregistered repo but never weaken a registry-enforced one (the flag is ignored when the registry already specifies a held-out). The operator surface for verifier-gated dispatch and the dogfood lives in the `autonomous-orchestration` skill, so Claude drives it on request rather than the user pasting CLI.

Worktree isolation (opt-in, `--worktree`): when set, `run_orchestrator` runs the attempt in a dedicated git worktree (`worktree.py`) on branch `orchestrator/<task-id>` instead of editing `--project` in place. One `work_dir` is computed at run start and threaded to EVERY tree-touching op (baseline_ref, reconcile, verify, tamper, held-out, the Worker's cwd), so they all follow the same tree (the silent-wrong-tree trap the roadmap names). Cleanup never `--force`s: a clean worktree is removed at the end (commits survive on the branch), while a dirty worktree or an escalated/failed run is retained for inspection. A non-git `--project` falls back to in-place with a warning; a real git failure fails the run loud rather than silently editing the tree the operator asked to isolate. This is the in-process N=1 isolation, distinct from the operator batch pattern below (one externally-created worktree per task passed as `--project`); use one or the other, not both.
